...
namespace Semantic.Controls
{
    public class WindowViewModel : ViewModelBase, IDialogServiceProvider
    {
        ...
        private Window childDialog = new DialogWindow();
        ...
        {
            this.CloseCommand = new DelegatedCommand(new Action(this.OnExecuteClose));
            this.MaximizeCommand = new DelegatedCommand(new Action(this.OnExecuteMaximize));
            this.MinimizeCommand = new DelegatedCommand(new Action(this.OnExecuteMinimize));
            this.EscapeCommand = new DelegatedCommand(new Action(this.OnExecuteEscape));
        }
        ...
        {
            foreach (Window window in new List<Window>(this._childWindows))
                window.Close();
        ...
        public bool ShowDialog(IDialog dialog)
        {
            ...
            if (dialog.CancelCommand == null)
                dialog.CancelCommand = new DelegatedCommand(() => this.DismissDialog(dialog))
                {
                    Name = "取消"
                };
            if (dialog == null || this.Dialog != null)
                return false;
            this.Dialog = dialog;
            childDialog.DataContext = dialog;
