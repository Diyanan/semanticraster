...
namespace Semantic.Controls
{
    public abstract class FilterClause
    {
        ...
        protected FilterClause(SerializableFilterClause state)
        {
            this.RequestedVersion = -1;
            this.CurrentVersion = -2;
        }
        ...
        protected void DispatchPropertiesUpdated()
        {
            ...
            {
                VisualizationTraceSource.Current.TraceEvent(TraceEventType.Information, 0, "DispatchPropertiesUpdated(): Calling OnPropertiesUpdated() for filterclause (table column = {0}, Agg Fn = {1}, CurrentVersion={2})", this.TableMember.Name, this.AggregationFunction, this.CurrentVersion);
                this.OnPropertiesUpdated(this, EventArgs.Empty);
            }
            else
                VisualizationTraceSource.Current.TraceEvent(TraceEventType.Information, 0, "DispatchPropertiesUpdated(): No subscribers for OnPropertiesUpdated() for filterclause (table column = {0}, Agg Fn = {1}, CurrentVersion={2})", this.TableMember.Name, this.AggregationFunction, this.CurrentVersion);
        }

        protected virtual void Wrap(SerializableFilterClause sfc)
        {
            sfc.AggregationFunction = this.AggregationFunction;
            sfc.TableMember = this.TableMember.Wrap();
        }

        internal virtual void Unwrap(SerializableFilterClause sfc)
        {
        ...
        internal virtual bool UpdateProperties(int queryVersion, ModelQueryColumn queryColumn)
        {
            ...
            {
                VisualizationTraceSource.Current.TraceEvent(TraceEventType.Information, 0, "UpdateProperties(): returning false for filterclause (table column = {0}, Agg Fn = {1}, queryVersion={2}, RequestedVersion={3})", this.TableMember.Name, this.AggregationFunction, queryVersion, this.CurrentVersion);
                return false;
            }
        }

        internal abstract SerializableFilterClause Wrap();
