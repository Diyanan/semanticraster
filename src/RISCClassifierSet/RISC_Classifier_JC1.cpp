...
/************************************************************************/
/*             RISC_Classifier_EJC1                                     */
/************************************************************************/

RISC_Classifier_JC1::RISC_Classifier_JC1(void)
{
}
...
int RISC_Classifier_JC1::Create()
{
	if (m_checkResult==1)
	{
...
//设置参数
void RISC_Classifier_JC1::SetParameter()
{
...
//训练样本
void RISC_Classifier_JC1::ExcuteTrain()
{
	int xx=0;
...
//执行分类
int RISC_Classifier_JC1::ExcuteClassify(double data[])
{
	try
	{
		int result=m_int1->Integrated_Execut_Classify_INT1(data,m_trainData_line-1);
		return result;
	}
	catch (GLC_Knn* e)
	{
...
//删除模型
int  RISC_Classifier_JC1::Destroy()
{
	try
	{
		if (m_int1!=NULL)
